...
function normalizeBaseUrl(raw) {
    ...
    if (!input)
        return exports.DEFAULTS.baseUrl;
    try {
...
function baseUrlAlreadyIncludesV1(baseUrl) {
    try {
...
function resolveBaseURL(s) {
    ...
    // Path 不留空：完全照抄
    if (prefixRaw)
        return `${base}${prefixRaw}`;
    // 留空：默认补 /v1；如果 baseUrl 已经包含 /v1，则不再重复追加
    if (baseUrlAlreadyIncludesV1(base))
        return base;
    return `${base}${defaultPathPrefix()}`;
}
function resolveChatCompletionsURL(s) {
    return `${resolveBaseURL(s)}/chat/completions`;
}
function splitEndpointToParts(endpoint) {
    try {
        ...
        return { baseUrl: exports.DEFAULTS.baseUrl, pathPrefix: exports.DEFAULTS.pathPrefix };
    }
}
function getSettings(opts) {
    const db = (0, sqlite_1.getDb)({ userDataPath: opts.userDataPath });
    const row = db.prepare('SELECT value FROM kv WHERE key = ?').get('settings.llm');
    if (!row?.value)
        return { ...exports.DEFAULTS };
    try {
        const parsed = JSON.parse(row.value);
        let baseUrl = typeof parsed.baseUrl === 'string' ? normalizeBaseUrl(parsed.baseUrl) : exports.DEFAULTS.baseUrl;
        let pathPrefix = typeof parsed.pathPrefix === 'string' ? normalizePathPrefix(parsed.pathPrefix) : exports.DEFAULTS.pathPrefix;
        ...
        const normalized = {
            baseUrl,
            pathPrefix,
            model: typeof parsed.model === 'string' ? parsed.model : exports.DEFAULTS.model,
            apiKey: typeof parsed.apiKey === 'string' ? parsed.apiKey : exports.DEFAULTS.apiKey,
            endpoint: typeof parsed.endpoint === 'string' ? parsed.endpoint : undefined
        };
        const needsWriteBack = parsed.baseUrl !== normalized.baseUrl ||
            parsed.pathPrefix !== normalized.pathPrefix ||
            (typeof parsed.endpoint === 'string' && (parsed.baseUrl == null || parsed.pathPrefix == null));
        if (needsWriteBack) {
            db.prepare('INSERT INTO kv(key, value, updated_at) VALUES(?, ?, ?)\n         ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_at=excluded.updated_at').run('settings.llm', JSON.stringify(normalized), Date.now());
        }
        return normalized;
    }
    catch {
...
function setSettings(opts, partial) {
    const current = getSettings({ userDataPath: opts.userDataPath });
    let next = { ...current, ...partial };
    if (typeof partial?.endpoint === 'string') {
        const parts = splitEndpointToParts(partial.endpoint);
    ...
    const db = (0, sqlite_1.getDb)({ userDataPath: opts.userDataPath });
    db.prepare('INSERT INTO kv(key, value, updated_at) VALUES(?, ?, ?)\n     ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_at=excluded.updated_at').run('settings.llm', JSON.stringify(next), Date.now());
    return next;
}
