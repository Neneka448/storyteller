...
import type { PipelineStepDef } from '../../pipeline/contracts'
import type { NodeHandler } from '../../nodes/contracts'

import { WorldKvNode, WorldKvSectionNode } from '../../nodes/impl/kvSteps'

export type WorldNodes = {
    overview: NodeHandler
    sections: Record<string, NodeHandler>
}

export function createWorldModule(artifactRepo: ArtifactRepo): { nodes: WorldNodes; steps: PipelineStepDef[] } {
    const overview = new WorldKvNode(artifactRepo)

    const sections: Array<{ key: string; stepId: string; title: string; type: string }> = [
    ...
    const sectionNodes: Record<string, NodeHandler> = {}
    for (const s of sections) {
        sectionNodes[s.key] = new WorldKvSectionNode(artifactRepo, { type: s.type, sectionTitle: s.title })
    }
