...
const schemaProvider: SchemaProvider<SandboxData> = {
...
class SandboxAgentTools implements AgentToolProvider {
    ...
    async invokeTool(toolName: string, args: any, ctx: ToolContext): Promise<any> {
        if (toolName !== 'sandboxSave') throw new Error('Unknown tool')
        ...
        if (!schemaProvider.validate(data)) throw new Error('Invalid sandbox payload')
...
export function createSandboxCapability(): NodeCapability<SandboxData> {
    return {
        id: 'sandbox',
        ...
        operation: {
            async execute(operation: string, args: any, currentData: SandboxData): Promise<SandboxData> {
                const base: any = currentData && typeof currentData === 'object' ? currentData : { html: '', css: '', js: '' }
                if (operation === 'set') {
                    return {
